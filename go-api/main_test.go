...
func TestMovingImages(t *testing.T) {
	...
	t.Run("should get data for existing keyword", func(t *testing.T) {
		url := movingImagesEndpoint + "?q=glasgow&page=1"
		e := echo.New()
		req, err := http.NewRequest(http.MethodPost, url, nil)
		req.Header.Set("Content-Type", "application/json")
		...
		if assert.NoError(t, movingImages(ec2fullurl, neptuneFullSparqlUrl, movingImagesendpoint)(c)) {
			assert.Equal(t, http.StatusOK, res.StatusCode, "http status code")
			assert.Contains(t, rec.Body.String(), "The search worked, there just aren't any results", "Not throwing error")
		}
	})

	t.Run("should test no keyword provided", func(t *testing.T) {
		url := movingImagesEndpoint + "?page=1"
		e := echo.New()
		req, err := http.NewRequest(http.MethodPost, url, nil)
		req.Header.Set("Content-Type", "application/json")
	...
	t.Run("should test to many keywords provided", func(t *testing.T) {
		url := movingImagesEndpoint + "?q=glasgow&page=1&test=1"
		e := echo.New()
		req, err := http.NewRequest(http.MethodPost, url, nil)
		req.Header.Set("Content-Type", "application/json")
		if err != nil {
